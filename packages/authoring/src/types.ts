...
export type AuthoringProgram =
  ...
  | { readonly kind: 'sequence'; readonly steps: readonly AuthoringProgram[] }
  | {
      readonly kind: 'when';
  ...
  | { readonly kind: 'repeat'; readonly count: number; readonly body: AuthoringProgram }
  | {
...
export type ActionInput = ActionInputBase &
  (
    | {
        readonly check: Extract<RpgIrCheck, { readonly kind: 'noRoll' }>;
        readonly rollScope?: never;
      }
    | {
        readonly check: Exclude<RpgIrCheck, { readonly kind: 'noRoll' }>;
