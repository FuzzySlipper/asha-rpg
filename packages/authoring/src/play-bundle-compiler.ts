...
export function preparePlayBundle(options: {
  ...
  validateSpatialSourceDefinitions(
    graph.materialized,
    graph.resolvedReferences,
    diagnostics,
  );
...
function validateRuleset(
  ...
  if (
    ...
    (!Number.isSafeInteger(
      ruleset.models.actionEconomy.acceptedActivationCeiling,
    ) ||
    ...
    diagnostics.push(
      diagnostic(
        'compatibility',
        'RULESET_ACTIVATION_CEILING_INVALID',
        '$.bundle.ruleset.models.actionEconomy.acceptedActivationCeiling',
        'the variable activation-budget model requires a ceiling from 1 through 64',
      ),
    );
  }
  ...
  if (ruleset.provides.activationBudgets.length > 8) {
    diagnostics.push(
      diagnostic(
        ...
        '$.bundle.ruleset.provides.activationBudgets',
        'a ruleset may declare at most 8 activation budgets',
      ),
    );
  }
  let previousBudgetId: string | undefined;
  const activationBudgetIds = new Set<string>();
  ...
  if (ruleset.provides.heterogeneousPoolProfiles.length > 16) {
    diagnostics.push(
      diagnostic(
        ...
        '$.bundle.ruleset.provides.heterogeneousPoolProfiles',
        'a Ruleset may declare at most 16 heterogeneous pool profiles',
      ),
    );
  }
  let previousPoolProfileId: string | undefined;
  for (const [profileIndex, profile] of ruleset.provides.heterogeneousPoolProfiles.entries()) {
    ...
    let previousAxisId: string | undefined;
    for (const [axisIndex, axis] of profile.axes.entries()) {
      if (
        !validPortableIdentifier(axis.id) ||
    ...
    const cancellationAxisIds = new Set<string>();
    let previousCancellationId: string | undefined;
    for (const [cancellationIndex, cancellation] of profile.cancellations.entries()) {
      if (
        !validPortableIdentifier(cancellation.id) ||
        ...
        diagnostics.push(
          diagnostic(
            ...
            `${path}.cancellations[${cancellationIndex}]`,
    ...
    const dieTypeIds = new Set<string>();
    let previousDieTypeId: string | undefined;
    for (const [dieIndex, die] of profile.dieTypes.entries()) {
      if (
        !validPortableIdentifier(die.id) ||
      ...
      for (const [faceIndex, face] of die.faces.entries()) {
        let previousFaceAxisId: string | undefined;
        const faceAxisIds = new Set<string>();
        if (
          face.value !== faceIndex + 1 ||
          ...
          diagnostics.push(
            diagnostic(
              ...
              `${path}.dieTypes[${dieIndex}].faces[${faceIndex}]`,
          ...
          if (
            ...
            diagnostics.push(
              diagnostic(
                ...
                `${path}.dieTypes[${dieIndex}].faces[${faceIndex}].vector[${vectorIndex}]`,
    ...
    let previousBandId: string | undefined;
    for (const [bandIndex, band] of profile.bands.entries()) {
      if (
        !validPortableIdentifier(band.id) ||
    ...
    for (const [ruleIndex, rule] of profile.outcomeRules.entries()) {
      ...
      let previousRequirementAxisId: string | undefined;
      for (const [requirementIndex, requirement] of rule.requirements.entries()) {
        if (
          !axisIds.has(requirement.axisId) ||
          ...
          diagnostics.push(
            diagnostic(
              ...
              `${rulePath}.requirements[${requirementIndex}]`,
...
function materializedCharacterClassData(
  definition: Extract<
    ContentDefinition,
...
function materializedPoolContributions(
  contributions: readonly import('./play-bundle-types.js').ContentPoolContribution[],
): readonly unknown[] {
  return contributions.map((contribution) => ({
    ...contribution,
    profile: {
...
function validateActionProcedureGraph(
  ...
  const spatialTriggerProcedures = new Set(
    records.flatMap((record) =>
      record.definition.kind === 'spatialSource'
        ? record.definition.spatialSource.triggers.map(
            (trigger) =>
              `${trigger.procedureOwnerPackageId}#${trigger.procedureId}`,
          )
        : [],
    ),
  );
    ...
    if (record.definition.kind === 'actionProcedure') {
      ...
      if (record.definition.implementation.kind === 'inline') {
        ...
        validateProcedureInlineTemplate(
          ...
          record,
          diagnostics,
          spatialTriggerProcedures.has(
            `${record.definition.ownerPackageId}#${record.definition.id}`,
          ),
        );
      } else {
  ...
  invocation: {
    ...
    readonly arguments: Readonly<
      Record<
        ...
        | import('./play-bundle-types.js').EquippedItemAttributeReference
      >
    >;
    readonly binding?: import('./play-bundle-types.js').EquippedItemBindingRequirement;
  },
...
function validateProcedureInlineTemplate(
  ...
  const materializedVariants = variants.map((variant) =>
    materializeProcedureTemplateForValidation(template, variant),
  );
  if (
    materializedVariants.some(
      (materialized) => !isStructurallyValidActionBody(materialized),
    )
  ) {
    diagnostics.push(
      diagnostic(
  ...
  if (
    materializedVariants.some(
      (materialized) =>
        !procedureActivationContractIsValid(
          materialized,
          ruleset,
          authorityTrigger,
        ),
    ...
    diagnostics.push(
      diagnostic(
        ...
        `${path}.activation`,
        'procedure action and reaction activations must match the selected action-economy model',
        profileDiagnosticContext(record),
      ),
    );
  }
...
function procedureActivationContractIsValid(
  body: unknown,
  ruleset: Ruleset,
  authorityTrigger: boolean,
): boolean {
  ...
  if (variableModel !== (body['activation'] !== undefined)) return false;
  return procedureProgramActivationContractIsValid(
    body['program'],
    variableModel,
  );
}

function procedureProgramActivationContractIsValid(
  program: unknown,
  variableModel: boolean,
): boolean {
  if (!isRecord(program) || typeof program['kind'] !== 'string') return false;
  switch (program['kind']) {
    case 'operation': {
    ...
    case 'sequence':
      return (
        ...
        program['steps'].every((step) =>
          procedureProgramActivationContractIsValid(step, variableModel),
        )
      );
    case 'when':
      return (
        procedureProgramActivationContractIsValid(
          program['then'],
          variableModel,
        ) &&
    ...
    case 'onCheck':
      return ['hit', 'miss', 'saved', 'failed', 'noRoll'].every(
        (branch) =>
          program[branch] === undefined ||
          procedureProgramActivationContractIsValid(
            program[branch],
            variableModel,
          ),
    ...
    case 'onOutcome': {
      ...
      return (
        isRecord(branches) &&
        Object.values(branches).every((branch) =>
          procedureProgramActivationContractIsValid(branch, variableModel),
        ) &&
        procedureProgramActivationContractIsValid(
          program['default'],
          variableModel,
...
function structurallyValidAreaTarget(
  targets: Readonly<Record<string, unknown>>,
): boolean {
  const area = targets['area'];
  if (targets['kind'] !== 'area') return area === undefined;
...
function validateEquippedItemBinding(
  binding: import('./play-bundle-types.js').EquippedItemBindingRequirement | undefined,
  argumentsById: Readonly<Record<string, unknown>>,
  record: DefinitionRecord,
  path: string,
  diagnostics: PlayBundleCompilerDiagnostic[],
): void {
  const attributeReferences = Object.entries(argumentsById)
    .filter(
      (entry): entry is [string, Readonly<Record<string, unknown>>] =>
        isRecord(entry[1]) && entry[1]['kind'] === 'equippedItemAttribute',
    )
  ...
  if (binding === undefined) {
    for (const reference of attributeReferences) {
      diagnostics.push(
        diagnostic(
          'source',
          'ACTION_ITEM_BINDING_REQUIRED',
          `${path}.arguments.${reference.argumentId}`,
          'equipped item attributes require an explicit action binding',
          profileDiagnosticContext(record),
        ),
      );
    }
  ...
  if (!validPortableIdentifier(binding.id)) {
    diagnostics.push(
      diagnostic(
        ...
        `${path}.binding.id`,
        'item binding identity must be a portable identifier',
        profileDiagnosticContext(record),
      ),
    );
  }
  for (const [field, values] of [
    ...
    if (!identifiersAreCanonical(values)) {
      diagnostics.push(
        diagnostic(
          'source',
          'ACTION_ITEM_BINDING_REQUIREMENT_INVALID',
          `${path}.binding.${field}`,
          `${field} must contain unique sorted portable identifiers`,
          profileDiagnosticContext(record),
        ),
      );
    }
  ...
  for (const reference of attributeReferences) {
    if (reference.value['bindingId'] === binding.id) continue;
    diagnostics.push(
      diagnostic(
        'source',
        'ACTION_ITEM_BINDING_REFERENCE_MISMATCH',
        `${path}.arguments.${reference.argumentId}.bindingId`,
        `item attribute reference must name binding ${binding.id}`,
        profileDiagnosticContext(record),
      ),
    );
  }
    ...
    if (
      ...
      diagnostics.push(
        diagnostic(
          ...
          path,
          'items may contain only schema, tags, traits, allowedSlots, typed attributes, and the closed typed contribution families',
          profileDiagnosticContext(record),
        ),
      );
    }
    for (const [field, values] of [
      ...
      if (!identifiersAreCanonical(values)) {
        diagnostics.push(
          diagnostic(
            'source',
            'ITEM_IDENTIFIERS_NOT_CANONICAL',
            `${path}.${field}`,
            `${field} must contain unique sorted portable identifiers`,
            profileDiagnosticContext(record),
          ),
    ...
    for (const [index, attribute] of item.attributes.entries()) {
      ...
      const valid = (() => {
        switch (attribute.type) {
          case 'boundedInteger':
            return (
              Object.keys(attribute).sort().join(',') ===
      ...
      diagnostics.push(
        diagnostic(
          'source',
          'ITEM_ATTRIBUTE_INVALID',
          attributePath,
          `item attribute ${attribute.id} does not satisfy ${attribute.type}`,
          profileDiagnosticContext(record),
        ),
      );
    }
...
function normalizeMaterializedActions(
  ...
  for (const [index, action] of result.artifact.actions.entries()) {
    ...
    if (action.check.kind === 'heterogeneousPool') {
      ...
      const profile =
        poolCheck.profile.rulesetId === bundle.ruleset.identity.id
          ? bundle.ruleset.provides.heterogeneousPoolProfiles.find(
              (candidate) => candidate.id === poolCheck.profile.id,
            )
          : undefined;
      if (profile === undefined) {
        diagnostics.push(
          diagnostic(
            ...
            `$.actions[${index}].check.profile`,
            'heterogeneous pool checks must reference a profile owned by the selected Ruleset',
          ),
        );
        continue;
      }
      ...
      const pairedAxisIds = new Set(
        profile.cancellations.flatMap((cancellation) => [
          cancellation.positiveAxisId,
          cancellation.negativeAxisId,
        ]),
      );
      const baseTotal = poolCheck.baseDice.reduce(
      ...
      if (
        ...
        baseTotal > 256 ||
        !identifiersAreCanonical(
          poolCheck.baseDice.map((term) => term.dieTypeId),
        ) ||
        poolCheck.baseDice.some(
          (term) =>
            !dieTypeIds.has(term.dieTypeId) ||
            !Number.isSafeInteger(term.count) ||
            term.count < 1,
        ) ||
        !identifiersAreCanonical(
          poolCheck.automaticAxes.map((axis) => axis.axisId),
        ) ||
        poolCheck.automaticAxes.some(
          (axis) =>
      ...
      if (
        ...
        diagnostics.push(
          diagnostic(
            'source',
            'ACTION_HETEROGENEOUS_POOL_BRANCHES_INVALID',
            `$.actions[${index}].program`,
            'pool checks require exactly one onOutcome program whose explicit branches are known bands and leave at least one band for default',
          ),
        );
      }
    }
...
function validateActionActivationContracts(
  action: RpgIrAction,
  ruleset: Ruleset,
  path: string,
  diagnostics: PlayBundleCompilerDiagnostic[],
  ...
  validateProgramActivationContracts(
    action.program,
    variableModel,
    ruleset,
    `${path}.program`,
...
function validateProgramActivationContracts(
  ...
  switch (program.kind) {
    case 'operation':
      if (program.operation.kind !== 'openReaction') return;
      for (const [index, option] of program.operation.options.entries()) {
        ...
        if (variableModel !== (option.activation !== undefined)) {
          diagnostics.push(
            diagnostic(
              'compatibility',
              'REACTION_ACTIVATION_MODEL_MISMATCH',
              activationPath,
              variableModel
...
function validateActivationCosts(
  activation: import('@asha-rpg/ir').RpgIrActivation,
  timing: import('@asha-rpg/ir').RpgIrActivationTiming,
  ruleset: Ruleset,
  path: string,
  diagnostics: PlayBundleCompilerDiagnostic[],
): void {
  if (activation.timing !== timing) {
    diagnostics.push(
      diagnostic(
        'compatibility',
        'ACTIVATION_TIMING_INVALID',
        `${path}.timing`,
        `activation timing must be ${timing}`,
      ),
    );
  }
  for (const [index, cost] of activation.costs.entries()) {
    const budget = ruleset.provides.activationBudgets.find(
      (candidate) => candidate.id === cost.budget.id,
    );
    if (
      cost.budget.rulesetId !== ruleset.identity.id ||
      budget === undefined ||
      ...
      diagnostics.push(
        diagnostic(
          'compatibility',
          'ACTIVATION_BUDGET_REFERENCE_INVALID',
          `${path}.costs[${index}].budget`,
          'activation costs must reference a same-owner budget with matching timing',
        ),
      );
      continue;
    }
    if (cost.amount > budget.initialAmount) {
      diagnostics.push(
        diagnostic(
          'source',
          'ACTIVATION_BUDGET_COST_UNREACHABLE',
          `${path}.costs[${index}].amount`,
          `activation cost ${cost.amount} exceeds budget ${budget.id} initial amount ${budget.initialAmount}`,
        ),
      );
    }
  }
...
function validateCharacterDefinitions(
  ...
  for (const record of materialized) {
    if (record.definition.kind === 'characterClass') {
      ...
      if (
        data.schema.identity !== 'asha.rpg.character-class' ||
        ...
        diagnostics.push(diagnostic(
          ...
          `${path}.schema`,
          'character classes require asha.rpg.character-class@1',
          { definitionId: record.definition.id, source: record.definition.source },
        ));
      }
      ...
      for (const [index, reference] of data.featureDefinitions.entries()) {
        ...
        if (
          !validPortableIdentifier(reference.definitionId) ||
          ...
          diagnostics.push(diagnostic(
            ...
            referencePath,
            'class feature references must be unique portable identities',
            { definitionId: record.definition.id, source: record.definition.source },
          ));
        }
        ...
        if (
          ...
          diagnostics.push(diagnostic(
            ...
            referencePath,
            `class feature ${reference.definitionId} must resolve to an exported characterFeature`,
            { definitionId: record.definition.id, source: record.definition.source },
          ));
        }
      }
    ...
    if (
      ...
      diagnostics.push(diagnostic(
        ...
        `${path}.schema`,
        'character features require asha.rpg.character-feature@4',
        { definitionId: record.definition.id, source: record.definition.source },
      ));
    }
    if (
...
function validateSpatialSourceDefinitions(
  records: readonly DefinitionRecord[],
  resolvedReferences: ReadonlyMap<string, readonly string[]>,
  diagnostics: PlayBundleCompilerDiagnostic[],
): void {
  ...
  const packageCounts = new Map<string, number>();
  const stackingPolicies = new Map<string, string>();
  for (const record of records) {
    if (record.definition.kind !== 'spatialSource') continue;
    const packageId = record.package.source.manifest.identity.id;
    packageCounts.set(packageId, (packageCounts.get(packageId) ?? 0) + 1);
    ...
    if (
      data.schema.identity !== 'asha.rpg.spatial-source' ||
      data.schema.version !== 1
    ) {
      diagnostics.push(diagnostic(
        'compatibility',
        ...
        `${path}.schema`,
        'spatial-source definitions require asha.rpg.spatial-source@1',
        profileDiagnosticContext(record),
      ));
    }
    if (
    ...
    if (!['all', 'allies', 'hostiles'].includes(data.targetFilter)) {
      diagnostics.push(diagnostic(
        'source',
        'SPATIAL_SOURCE_TARGET_FILTER_INVALID',
        `${path}.targetFilter`,
        'the target filter must be all, allies, or hostiles',
        profileDiagnosticContext(record),
      ));
    }
    if (!validPortableIdentifier(data.stackingId)) {
      diagnostics.push(diagnostic(
        ...
        `${path}.stackingId`,
        'spatial-source stacking identity must be portable',
        profileDiagnosticContext(record),
      ));
    }
    const previousPolicy = stackingPolicies.get(data.stackingId);
    ...
    stackingPolicies.set(data.stackingId, data.stacking);
    if (
      data.tenure.kind !== 'fixed' ||
      !['globalTurnTransition', 'roundTransition', 'sourceTurnStart'].includes(
        data.tenure.anchor,
      ) ||
      ...
      diagnostics.push(diagnostic(
        ...
        `${path}.tenure`,
        'spatial-source tenure must be a supported fixed anchor with count 1..=1000',
        profileDiagnosticContext(record),
      ));
    }
    ...
    let previousBoundaryOrder: number | undefined;
    const sourceReferences =
      resolvedReferences.get(globalDefinitionId(record)) ?? [];
    for (const [index, trigger] of data.triggers.entries()) {
      ...
      const procedureRecord = sourceReferences
        ...
        .find(
          (candidate) =>
            candidate?.definition.id === trigger.procedureId &&
            candidate.package.source.manifest.identity.id ===
              trigger.procedureOwnerPackageId,
        );
      if (
        procedureRecord?.definition.kind !== 'actionProcedure' ||
        ...
        diagnostics.push(diagnostic(
          ...
          `${triggerPath}.procedureId`,
          'trigger procedures must resolve through the closed graph, have the exact owner package, and declare no parameters',
          profileDiagnosticContext(record),
        ));
      }
    }
  ...
  for (const [packageId, count] of packageCounts) {
    if (count > 128) {
      diagnostics.push(diagnostic(
        ...
        '$.materializedDefinitions',
        `package ${packageId} has ${count} spatial sources; maximum is 128`,
        {},
      ));
    }
  }
...
function validatePoolContributions(
  contributions: readonly import('./play-bundle-types.js').ContentPoolContribution[],
  path: string,
  record: DefinitionRecord,
  ruleset: Ruleset,
  ...
  for (const [index, contribution] of contributions.entries()) {
    const contributionPath = `${path}[${index}]`;
    const profile =
      contribution.profile.rulesetId === ruleset.identity.id
        ? ruleset.provides.heterogeneousPoolProfiles.find(
            (candidate) => candidate.id === contribution.profile.id,
          )
        : undefined;
    if (
    ...
    if (
      ...
      diagnostics.push(
        diagnostic(
          'graph',
          'POOL_CONTRIBUTION_CONTRACT_INVALID',
          contributionPath,
          'pool contributions require canonical ids and owner-local profile and stacking-group references',
          profileDiagnosticContext(record),
        ),
      );
    }
...
function validateContributionPredicate(
  ...
  if (condition.kind === 'actorSurrounded') {
    if (
      !Number.isInteger(condition.minimumHostiles) ||
...
function resolvedDefinitionReference(
  ...
  const packageKey =
    reference.importAs === undefined
      ? source.package.key
      : source.package.aliases.get(reference.importAs);
  if (packageKey === undefined) return undefined;
  const targetGlobalId = `${packageKey}#${reference.definitionId}`;
  if (
    !(resolvedReferences.get(globalDefinitionId(source)) ?? []).includes(
      targetGlobalId,
    )
  ) {
    return undefined;
...
function validateParticipantProfiles(
  materialized: readonly DefinitionRecord[],
  resolvedReferences: ReadonlyMap<string, readonly string[]>,
  ruleset: Ruleset,
  diagnostics: PlayBundleCompilerDiagnostic[],
  ...
  const rulesetValues = new Map(
    ruleset.provides.values.map((value) => [`${value.kind}:${value.id}`, value]),
  );
  const numericDomains = new Map(
    ruleset.provides.numericDomains.map((domain) => [domain.id, domain]),
  );

  for (const record of materialized) {
    if (!isParticipantProfileDefinition(record.definition)) continue;
    ...
    if (profile === undefined) {
      diagnostics.push(diagnostic(
        'source',
        'PARTICIPANT_PROFILE_SCHEMA_INVALID',
        path,
        'participant profiles require asha.rpg.participant-profile@2, a role, explicit class/feature selection, definition references, and typed base capabilities',
        profileDiagnosticContext(record),
      ));
      continue;
    }
    ...
    for (const [index, reference] of profile.definitionReferences.entries()) {
      ...
      if (definitionIdentities.has(identity)) {
        diagnostics.push(diagnostic(
          'source',
          'PARTICIPANT_PROFILE_DEFINITION_DUPLICATE',
          `${path}.definitionReferences[${index}]`,
          `participant profile repeats definition ${reference.definitionId}`,
          profileDiagnosticContext(record),
        ));
      }
      ...
      if (!target.exported || target.definition.visibility !== 'public') {
        diagnostics.push(diagnostic(
          ...
          `${path}.definitionReferences[${index}]`,
          `profile definition ${reference.definitionId} is not exported for participant setup`,
          profileDiagnosticContext(record),
        ));
      }
      if (target.definition.kind === 'action') hasAction = true;
    }
    if (!hasAction) {
      diagnostics.push(diagnostic(
        'compatibility',
        'PARTICIPANT_PROFILE_ACTION_REQUIRED',
        `${path}.definitionReferences`,
        'an exported participant profile must reference at least one action',
        profileDiagnosticContext(record),
      ));
    }
    ...
    if (
      ...
      diagnostics.push(diagnostic(
        ...
        `profile class ${profile.classDefinition.definitionId} must resolve to an exported characterClass`,
        profileDiagnosticContext(record),
      ));
    }
    if (
    ...
    const classFeatureTargets =
      selectedClass?.definition.kind === 'characterClass'
        ? new Set(
            selectedClass.definition.characterClass.featureDefinitions
              .map((reference) =>
                resolvedDefinitionReference(
                  selectedClass,
                  reference,
              ...
              .filter(
                (target): target is DefinitionRecord => target !== undefined,
              )
              .map(globalDefinitionId),
          )
        : new Set<string>();
    const selectedFeatureIdentities = new Set<string>();
    for (const [index, reference] of profile.featureDefinitions.entries()) {
      ...
      if (selectedFeatureIdentities.has(identity)) {
        diagnostics.push(diagnostic(
          ...
          featurePath,
          `participant profile repeats feature ${reference.definitionId}`,
          profileDiagnosticContext(record),
        ));
      }
      ...
      if (
        ...
        diagnostics.push(diagnostic(
          ...
          featurePath,
          `profile feature ${reference.definitionId} must resolve to an exported characterFeature`,
          profileDiagnosticContext(record),
        ));
        continue;
      }
      if (!classFeatureTargets.has(globalDefinitionId(target))) {
        diagnostics.push(diagnostic(
          ...
          featurePath,
          `selected class does not make character feature ${reference.definitionId} available`,
          profileDiagnosticContext(record),
        ));
      }
    }

    const itemInstances = new Map<string, DefinitionRecord>();
    for (const [index, item] of profile.items.entries()) {
      const itemPath = `${path}.items[${index}]`;
      if (
        !validPortableIdentifier(item.id) ||
        itemInstances.has(item.id)
      ) {
        diagnostics.push(diagnostic(
          'source',
          'PARTICIPANT_PROFILE_ITEM_INSTANCE_ID_INVALID',
          `${itemPath}.id`,
          'profile item instance identities must be unique portable identifiers',
          profileDiagnosticContext(record),
        ));
      }
      const targetPackageKey =
        item.definition.importAs === undefined
          ? record.package.key
          : record.package.aliases.get(item.definition.importAs);
      const targetGlobalId =
        targetPackageKey === undefined
          ? undefined
          : `${targetPackageKey}#${item.definition.definitionId}`;
      const target =
        targetGlobalId !== undefined && resolved.includes(targetGlobalId)
          ? recordsByGlobalId.get(targetGlobalId)
          : undefined;
      if (target === undefined) continue;
      if (
        target.definition.kind !== 'item' ||
        !target.exported ||
        target.definition.visibility !== 'public'
      ) {
        diagnostics.push(diagnostic(
          ...
          `${itemPath}.definition`,
          `profile item ${item.definition.definitionId} must resolve to an exported item definition`,
          profileDiagnosticContext(record),
        ));
        continue;
      }
    ...
    for (const [index, equipment] of profile.equipment.entries()) {
      ...
      if (
        !validPortableIdentifier(equipment.slotId) ||
        equipmentSlots.has(equipment.slotId)
      ) {
        diagnostics.push(diagnostic(
          'source',
          'PARTICIPANT_PROFILE_EQUIPMENT_SLOT_INVALID',
          `${equipmentPath}.slotId`,
          'profile equipment slot identities must be unique portable identifiers',
          profileDiagnosticContext(record),
        ));
      }
      ...
      if (item === undefined) {
        diagnostics.push(diagnostic(
          ...
          `${equipmentPath}.itemInstanceId`,
          `equipment references unknown item instance ${equipment.itemInstanceId}`,
          profileDiagnosticContext(record),
        ));
      } else if (
        item.definition.kind === 'item' &&
        ...
        diagnostics.push(diagnostic(
          ...
          `${equipmentPath}.slotId`,
          `item ${item.definition.id} cannot occupy slot ${equipment.slotId}`,
          profileDiagnosticContext(record),
        ));
      }
    }
    ...
    for (const [index, capability] of profile.capabilities.entries()) {
      ...
      if (capabilityIdentities.has(identity)) {
        diagnostics.push(diagnostic(
          ...
          capabilityPath,
          `participant profile repeats capability fact ${identity}`,
          profileDiagnosticContext(record),
        ));
      }
      ...
      if (capability.owner === 'stat' || capability.owner === 'defense') {
        const ownership = rulesetValueOwnershipOf(capability).find(
          (candidate) => candidate.field === 'id',
        );
        if (ownership === undefined) {
          diagnostics.push(diagnostic(
            'compatibility',
            'PARTICIPANT_PROFILE_RULESET_VALUE_OWNER_MISSING',
            `${capabilityPath}.id`,
            `profile ${capability.owner} ${capability.id} must be authored from a nominal Ruleset reference`,
            profileDiagnosticContext(record),
          ));
        }
        ...
        if (contract.source.kind === 'derived') {
          diagnostics.push(diagnostic(
            ...
            capabilityPath,
            `profile must not supply derived Ruleset value ${capability.id}`,
            profileDiagnosticContext(record),
          ));
        }
        ...
        if (
          ...
          diagnostics.push(diagnostic(
            ...
            `${capabilityPath}.value`,
            `profile value must be within ${domain.minimum}..=${domain.maximum}`,
            profileDiagnosticContext(record),
          ));
        }
        continue;
      }
      const ownership = catalogOwnershipOf(capability).find(
        (candidate) => candidate.field === 'id',
      );
      if (ownership === undefined || ownership.category !== capability.owner) {
        diagnostics.push(diagnostic(
          'compatibility',
          'PARTICIPANT_PROFILE_CONTENT_VALUE_OWNER_MISSING',
          `${capabilityPath}.id`,
          `profile ${capability.owner} ${capability.id} must be authored from a nominal Content Pack catalog reference`,
          profileDiagnosticContext(record),
        ));
      }
      if (capability.owner === 'resource') {
        ...
        capability.stackingGroup.trim().length === 0 ||
        ...
        diagnostics.push(diagnostic(
          ...
          capabilityPath,
          'profile modifiers require identities and remainingTurns within 1..=1000',
          profileDiagnosticContext(record),
        ));
      }
    }
...
function isParticipantProfileDefinition(
  definition: ContentDefinition,
): definition is Extract<ContentDefinition, { readonly kind: 'support' }> {
  return definition.kind === 'support' && definition.semantic.catalog === 'participantProfile';
}

function authoredParticipantProfile(
  definition: ContentDefinition,
): import('./play-bundle-types.js').ContentParticipantProfileData | undefined {
  if (!isParticipantProfileDefinition(definition)) return undefined;
  const data = definition.semantic.data;
  if (!isRecord(data) || !isRecord(data['schema'])) return undefined;
  const schema = data['schema'];
  if (
    schema['identity'] !== 'asha.rpg.participant-profile' ||
    schema['version'] !== 2 ||
  ...
  return data as unknown as import('./play-bundle-types.js').ContentParticipantProfileData;
}

function isProfileItem(value: unknown): boolean {
  return (
    isRecord(value) &&
    typeof value['id'] === 'string' &&
...
function isProfileEquipment(value: unknown): boolean {
  return (
    isRecord(value) &&
...
function isProfileDefinitionReference(value: unknown): boolean {
  if (!isRecord(value) || typeof value['definitionId'] !== 'string') return false;
  return value['importAs'] === undefined || typeof value['importAs'] === 'string';
}

function isProfileCapability(value: unknown): boolean {
  if (!isRecord(value) || typeof value['owner'] !== 'string') return false;
  if (value['owner'] === 'vitality') return isProfileBoundedValue(value['value']);
  ...
  if (value['owner'] === 'modifier') {
    return (
      typeof value['stackingGroup'] === 'string' &&
      typeof value['id'] === 'string' &&
...
function isProfileBoundedValue(value: unknown): boolean {
  return (
    isRecord(value) &&
...
function profileCapabilityIdentity(
  capability: import('./play-bundle-types.js').ScenarioInitialCapability,
): string {
  if (capability.owner === 'vitality') return 'vitality';
...
function profileCapabilityRequirement(
  owner: import('./play-bundle-types.js').ScenarioInitialCapability['owner'],
): RpgCapabilityId | undefined {
  switch (owner) {
...
function validateProfileBoundedValue(
  value: import('./play-bundle-types.js').ScenarioBoundedValue,
  path: string,
  record: DefinitionRecord,
  diagnostics: PlayBundleCompilerDiagnostic[],
): void {
  if (
    ...
    diagnostics.push(diagnostic(
      ...
      path,
      'profile bounded values require integer 0 <= current <= max',
      profileDiagnosticContext(record),
    ));
  }
}

function profileDiagnosticContext(record: DefinitionRecord) {
  return {
    packageId: record.package.source.manifest.identity.id,
    definitionId: record.definition.id,
    source: record.definition.source,
  ...
  for (const record of materialized) {
    ...
    for (const capability of profile.capabilities) {
      ...
      if (capabilityId !== undefined) {
        ...
        if (version !== undefined) capabilities.set(capabilityId, version);
      }
      if (capability.owner !== 'stat' && capability.owner !== 'defense') continue;
      const key = `${capability.owner}:${capability.id}`;
      values.add(key);
      const contract = rulesetValues.get(key);
      if (contract !== undefined) numericDomains.add(contract.numericDomainId);
    }
  }
  if (context.diagnostics.length > 0) return undefined;
  return {
...
function materializedSupportSemantic(
  definition: Extract<ContentDefinition, { readonly kind: 'support' }>,
): unknown {
  const profile = authoredParticipantProfile(definition);
  if (profile === undefined) return definition.semantic;
  return {
    ...definition.semantic,
    data: {
      ...
      items: profile.items
        ...
        .sort((left, right) => left.id.localeCompare(right.id)),
      equipment: [...profile.equipment].sort((left, right) =>
        left.slotId.localeCompare(right.slotId),
      ),
      capabilities: profile.capabilities,
    } satisfies import('./play-bundle-types.js').MaterializedParticipantProfileData,
  };
}
